...
- красивый web-интерфейс: `http://127.0.0.1:8000/`
...
Главная страница проекта - это каталог `BookScope`.

Что можно делать в интерфейсе:

- смотреть книги из SQLite-базы в виде карточек;
- искать книгу по части названия;
- фильтровать по категории, рейтингу, цене и наличию;
- выбирать конкретный запуск скрапера;
- сортировать выдачу по дате, цене, рейтингу, названию или категории;
- открывать историю цены одной книги между разными запусками.

Если база еще не создана, интерфейс покажет понятное сообщение. Чтобы заполнить базу,
сначала нужно выполнить scraping-команду из раздела выше.
